...
from audit_log import (
    ...
    EVENT_ROLE_CHANGED, EVENT_ADMIN_ACTION, EVENT_LOGIN, EVENT_LOGOUT,
    EVENT_LOGIN_FAILED, SEVERITY_INFO, SEVERITY_WARNING, SEVERITY_ERROR
)
...
async def home(request: Request):
    return templates.TemplateResponse("modern-login.html", {"request": request})
...
async def login(request: Request, username: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    if not validate_username(username):
        ...
        log_audit_event(
            ...
            details={"reason": "invalid_username_format", "username": username},
            ip_hash=client_ip,
            severity=SEVERITY_WARNING
        )
        return templates.TemplateResponse("modern-login.html", {"request": request, "error": "Invalid credentials"})
    ...
    if user.is_active == 0:
        ...
        log_audit_event(
            event_type=EVENT_LOGIN_FAILED,
            details={"reason": "account_disabled", "username": username, "user_id": user.id},
            ip_hash=client_ip,
            username=username,
            user_id=user.id,
            severity=SEVERITY_WARNING
        )
        return templates.TemplateResponse("modern-login.html", {"request": request, "error": "Account is disabled. Contact administrator."})
    ...
    if user.failed_login_attempts >= 5:
        ...
        log_audit_event(
            event_type=EVENT_LOGIN_FAILED,
            details={"reason": "account_locked", "username": username, "user_id": user.id, "attempts": user.failed_login_attempts},
            ip_hash=client_ip,
            username=username,
            user_id=user.id,
            severity=SEVERITY_WARNING
        )
        return templates.TemplateResponse("modern-login.html", {"request": request, "error": "Account locked due to too many failed attempts. Contact administrator."})
    ...
    if not verify_password(password, user.hashed_password):
        ...
        log_audit_event(
            event_type=EVENT_LOGIN_FAILED,
            details={"reason": "invalid_password", "username": username, "user_id": user.id, "attempts": user.failed_login_attempts},
            ip_hash=client_ip,
            username=username,
            user_id=user.id,
            severity=SEVERITY_WARNING
    ...
    log_audit_event(
        ...
        details={"username": username, "user_id": user.id, "admin_level": user.is_admin},
        ip_hash=client_ip,
        username=username,
        user_id=user.id,
...
async def register_page(request: Request):
    return templates.TemplateResponse("modern-login.html", {"request": request, "error": "Registration is disabled. Contact administrator."})
...
async def chat_page(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return templates.TemplateResponse("modern-chat.html", {
            "request": request, 
            "username": username,
            "is_admin": is_admin
        })
...
async def logout(request: Request):
    ...
    # Log logout event if we have a valid token
    if token:
        try:
            username = verify_token(token.replace("Bearer ", ""))
            if username:
...
async def admin_panel(request: Request, current_user: User = Depends(require_super_admin)):
    return templates.TemplateResponse("modern-admin.html", {"request": request, "username": current_user.username})

# Initialize admin endpoints
admin_endpoints.init_admin_module(
    templates_instance=templates,
    db_dependency=get_db,
    user_model=User,
    super_admin_dependency=require_super_admin,
    password_hash_function=get_password_hash
)
